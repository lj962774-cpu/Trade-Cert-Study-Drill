...
| 영역 | 내용 |
|------|------|
| **학습(읽기)** | 채점 없이 앞·뒤를 함께 읽으며 익히는 모드. 태그 필터·순서/무작위·정답 가림(Space) 지원, `←`/`→` 로 이동 |
| **복습(SRS)** | 라이트너 5-박스. 정답 → `box+1`(최대 5), 오답 → `box=1` + 10분 뒤 재등장. 간격일 `{1:0, 2:1, 3:3, 4:7, 5:16}` 은 설정에서 조정 |
...
| **HS 트리 드릴** | 부(21)→류(96, WCO 표준)→호 계층. **류·호 두 범위** × 정방향(이름→코드)·역방향(코드→이름) 드릴 |
...
**복습(SRS)**: `Space` / `Enter` — 정답 보기 · `1` — 틀림 · `2` — 맞음
**학습(읽기)**: `←` / `→` — 이전/다음 · `Space` — 정답 가림 시 펼치기 / 아니면 다음
